...
public class ARRenderer {
    ...
    /**
     * Check if AR goggles are available — either in the vanilla armor slot or
     * Curios head slot.
     */
    private static boolean isArAvailable() {
        ...
        try {
            Optional<ICurioStacksHandler> curiosOpt = CuriosApi.getCuriosInventory(player)
                    .resolve()
                    .flatMap(handler -> Optional.ofNullable(handler.getCurios().get("head")));

            if (curiosOpt.isPresent()) {
                ICurioStacksHandler stacksHandler = curiosOpt.get();
                for (int i = 0; i < stacksHandler.getSlots(); i++) {
                    if (stacksHandler.getStacks().getStackInSlot(i).getItem() == CCARItems.AR_GOGGLES.get()) {
                        return true;
                    }
                }
            }
        } catch (Exception ignored) {
            // Curios API not available — mod loaded without Curios
        }
    ...
    public static final IGuiOverlay AR_OVERLAY = (gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        elements.forEach((id, e) -> {
            if (!isArAvailable)
                return;
            if (e instanceof ARElement2D element2D)
                element2D.render(gui, guiGraphics, partialTick, screenWidth, screenHeight);
        });
    };
